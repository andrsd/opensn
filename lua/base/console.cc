...
Console::Console(ConsoleApp& app) noexcept : app_(app), console_state_(luaL_newstate())
{
  luaL_openlibs(console_state_);
  PostMPIInfo(app.LocationID(), app.ProcessCount());
}
...
Console::LuaWrapperCall(lua_State* L)
{
  ...
  //
  // chi::ParameterBlock main_arguments_block;
...
{
  App().Log().Log() << "Console loop started. Type \"exit\" to quit (or Ctrl-C).";

  const bool HOME = App().LocationID() == 0;
  while (true)
  {
    std::string console_input;

    if (HOME) getline(std::cin, console_input); // Home will be waiting here

    int console_input_len = ConsoleInputNumChars(console_input);
    MPI_Bcast(&console_input_len, 1, MPI_INT, 0, App().Comm());
    if (console_input_len >= 0)
    {
      if (HOME)
      {
        void* raw_chars = (void*)console_input.c_str();
        MPI_Bcast(raw_chars, console_input_len, MPI_CHAR, 0, App().Comm());
      }
      else
      {
        std::vector<char> raw_chars(console_input_len + 1, '\0');
        MPI_Bcast(raw_chars.data(), console_input_len, MPI_CHAR, 0, App().Comm());
        console_input = std::string(raw_chars.data());
      }
    }
    else
      break;
  ...
  App().Log().Log() << "Console loop stopped successfully.";
}
...
Console::ExecuteFile(const std::string& fileName, int argc, char** argv) const
{
  ...
  if (not fileName.empty())
  {
    ...
    if (error > 0)
    {
      App().Log().LogAllError() << "LuaError: " << lua_tostring(this->console_state_, -1);
      return EXIT_FAILURE;
    }
  }
  return EXIT_SUCCESS;
...
void
Console::SetLuaConstant(const std::string& constant_name, const chi_data_types::Varying& value)
{
  // auto& console = GetInstance();
...
int
Console::ConsoleInputNumChars(const std::string& input) const
{
...
void
Console::LuaDoString(const std::string& the_string) const
{
  bool error = luaL_dostring(console_state_, the_string.c_str());
  if (error)
  {
    App().Log().LogAll() << lua_tostring(console_state_, -1);
