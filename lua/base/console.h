...
#define RegisterLuaFunctionAsIs(func_name)                                                         \
  static char ConsoleJoinWordsB(unique_var_name_luacfunc_##func_name##_, __COUNTER__) =            \
    Console::AddFunctionToRegistryGlobalNamespace(#func_name, func_name)
 ...
 * \param func_name NonQuotedString. The name of the function as it will appear in the lua console.
 */
#define RegisterLuaFunction(function, namespace_name, func_name)                                   \
  static char ConsoleJoinWordsB(unique_var_name_luacfunc_##func_name##_, __COUNTER__) =            \
    Console::AddFunctionToRegistryInNamespaceWithName(function, #namespace_name, #func_name)
...
#define RegisterWrapperFunction(namespace_name, name_in_lua, syntax_function, actual_function)     \
  static char ConsoleJoinWordsB(unique_var_name_luacfunc_##name_in_lua##_, __COUNTER__) =          \
    Console::AddWrapperToRegistryInNamespaceWithName(                                              \
...
#define RegisterLuaConstant(namespace_name, name_in_lua, value)                                    \
  static char ConsoleJoinWordsB(unique_var_name_luaconst_##namespace_name##_##name_in_lua,         \
                                __COUNTER__) =                                                     \
    Console::AddLuaConstantToRegistry(#namespace_name, #name_in_lua, value)
...
#define RegisterLuaConstantAsIs(name_in_lua, value)                                                \
  static char ConsoleJoinWordsB(unique_var_name_luaconst_##name_in_lua, __COUNTER__) =             \
    Console::AddLuaConstantToRegistry("", #name_in_lua, value)
...
public:
  explicit Console(ConsoleApp& app) noexcept;

  ConsoleApp& App() const;
...
public:
   ...
   */
  static chi::InputParameters DefaultGetInParamsFunc();
...
private:
  ...
  int ConsoleInputNumChars(const std::string& input) const;

  /**
   * Executes a string within the lua-console.
   */
  void LuaDoString(const std::string& the_string) const;

  /// Reference to an application using this console
  ConsoleApp& app_;
