...
std::vector<double>
LuaResponseFunction::Evaluate(int num_groups, const opensn::Vector3& pos, int mat_id) const
{
  ...
  if (lua_return.size() > response.size())
    throw std::logic_error(fname + " Call lua-function, " + lua_function_name_ +
