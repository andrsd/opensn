...
chiCreateProductQuadrature(lua_State* L)
{
  ...
  if (ident == (int)chi_math::ProductQuadratureType::GAUSS_LEGENDRE)
  {
    ...
    //                  << " azimuthal angles and " << new_quad->polar_ang_.size() << " polar
    //                  angles.";
    // }
  ...
  else if (ident == (int)chi_math::ProductQuadratureType::CUSTOM_QUADRATURE)
  {
    ...
    {
      // FIXME
      // Chi::log.LogAllError() << "chiCreateProductQuadrature:CUSTOM_QUADRATURE, third argument
      // must "
      //                        << "be a lua table.";
      opensn::App::Exit(EXIT_FAILURE);
    }
    ...
    //
    // auto new_quad =
    //   std::make_shared<chi_math::AngularQuadratureProdCustom>(azimuthal, polar, weights,
    //   verbose);
    //
    // Chi::angular_quadrature_stack.push_back(new_quad);
    // const size_t index = Chi::angular_quadrature_stack.size() - 1;
  ...
  {
    // FIXME
    // Chi::log.LogAllError() << "In call to chiCreateProductQuadrature. Unsupported quadrature
    // type"
    //                           " supplied. Given: "
    //                        << ident;
    opensn::App::Exit(EXIT_FAILURE);
