...
Meshing in openSn is made available via the `mesh` namespace.
All instances of entities are stored within a handler (`mesh::MeshHandler`).
A mesh handler is created with a call to `MeshHandlerCreate()`.
