...
called `OpenSnRegisterObjectNamespace(namespace_name, obj_name)`. The arguments to this
macro, `namespace_name` and `obj_name`, then get used to construct the following
\code
OpenSnRegisterObjectNamespace(namespace_name, ObjectName);
//
// becomes:
//
...
OpenSnRegisterObjectNamespace(unit_tests, CoolObject);
