...
nodes to create a 2D geometry with square cells.

We also partition the 2D mesh into 2x2 subdomains using `KBAGraphPartitioner`. Since we want the split the x-axis in 2,
we give only 1 value in the xcuts array. Likewise for ycuts. The assignment to a partition is done based on where the
...
--[[ @doc
### Options for the Linear Boltzmann Solver
In the LBS options, we pass the maximum scattering order to be employed (should be equal or less than the one
supplied in the cross-section file)
--]]
lbs_options =
{
  scattering_order = 0,
...
indexing starts at 1) and export it to a VTK file whose name is supplied by the user.
