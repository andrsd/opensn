...
XXPowerIterationKEigenSCDSA::XXPowerIterationKEigenSCDSA(opensn::App& app,
                                                         const chi::InputParameters& params)
  : XXPowerIterationKEigen(app, params),
...
XXPowerIterationKEigenSCDSA::Initialize()
{
  ...
  else
  {
    ...
    diffusion_solver_ =
      std::make_shared<acceleration::DiffusionPWLCSolver>(App(),
                                                          std::string(TextName() + "_WGDSA"),
                                                          *continuous_sdm_ptr_,
                                                          uk_man,
                                                          bcs,
  ...
  App().Log().Log() << "Initializing diffusion solver";
  diffusion_solver_->Initialize();
  App().Barrier();
  App().Log().Log() << "Done Initializing diffusion solver";

  App().Log().Log() << "Assembling A and b";
  std::vector<double> dummy_rhs;
  ...
  diffusion_solver_->AssembleAand_b(dummy_rhs);
  App().Log().Log() << "Done Assembling A and b";
}
...
XXPowerIterationKEigenSCDSA::Execute()
{
  ...
  const size_t tag_SCDSA_solve_time = App().Log().GetRepeatingEventTag("SCDSA_solve_time");
  const size_t tag_sweep_timing = App().Log().GetRepeatingEventTag("Sweep Timing");
  ...
  while (nit < max_iters_)
  {
    ...
    front_wgs_context_->ApplyInverseTransportOperator(Source()); // Sweep
    ...
    for (size_t k = 0; k < accel_pi_max_its_; ++k)
    {
      ...
      for (int i = 0; i < 1; ++i)
      {
        ...
        App().Log().LogEvent(tag_SCDSA_solve_time, chi::ChiLog::EventType::EVENT_BEGIN);
        diffusion_solver_->Assemble_b(Ss + Sfaux + Ss_res - Sf0_ell);
        diffusion_solver_->Solve(epsilon_kp1, true);
        App().Log().LogEvent(tag_SCDSA_solve_time, chi::ChiLog::EventType::EVENT_END);
      ...
      if (accel_pi_verbose_ >= 1)
        App().Log().Log() << "PISCDSA iteration " << k << " lambda " << lambda_kp1
  ...
  App().Log().Log() << "        Final change          :        " << std::setprecision(6)
                    ...
                    << App().Log().ProcessEvent(tag_SCDSA_solve_time,
                                                chi::ChiLog::EventOperation::TOTAL_DURATION) *
                         1.0e-6
                    << "s\n"
                    << "        Total sweep time      :        "
                    << App().Log().ProcessEvent(tag_sweep_timing,
                                                chi::ChiLog::EventOperation::TOTAL_DURATION);
  App().Log().Log() << "\n";
                                                      ...
                                                      const chi_math::UnknownManager& uk_man)
{
  ...
  auto vgc = std::make_shared<chi_math::VectorGhostCommunicator>(
    num_local_dofs, num_globl_dofs, global_indices, App().Comm());
  ...
  App().Log().Log() << "Done making PWLD ghost communicator";
