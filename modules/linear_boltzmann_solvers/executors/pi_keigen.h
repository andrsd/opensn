...
protected:
  ...
  Vector<double>& q_moments_local_;
  Vector<double>& phi_old_local_;
  Vector<double>& phi_new_local_;
...
protected:
  ...
  void SetLBSFissionSource(const Vector<double>& input, bool additive);

  /// Combines function calls to set scattering source source.
  void
  SetLBSScatterSource(const Vector<double>& input, bool additive, bool suppress_wg_scat = false);
