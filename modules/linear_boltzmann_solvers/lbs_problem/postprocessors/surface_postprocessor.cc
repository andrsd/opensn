                         ...
                         : std::nullopt)
{
  ...
  values_.resize({static_cast<int>(n_lvs), static_cast<int>(groups_.size())});
}
  ...
  for (unsigned int i = 0; i < side_faces_.size(); ++i)
  {
    switch (value_type_)
    {
      case ValueType::INTEGRAL:
        MemCopyRow(values_, i, ComputeIntegral(side_faces_[i]));
        break;

      case ValueType::MAX:
        MemCopyRow(values_, i, ComputeMax(side_faces_[i]));
        break;

      case ValueType::MIN:
        MemCopyRow(values_, i, ComputeMin(side_faces_[i]));
        break;

      case ValueType::AVERAGE:
        MemCopyRow(values_, i, ComputeAvg(side_faces_[i]));
        break;
    }
  }
}
...
SurfacePostprocessor::ComputeIntegral(const std::vector<SideFace>& side_faces)
{
  ...
  for (const auto [cell_id, face_idx] : side_faces)
  {
    ...
    const auto num_face_nodes = cell_mapping.GetNumFaceNodes(face_idx);
