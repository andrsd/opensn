...
VolumetricSource::GetInputParameters()
{
  ...
  params.AddOptionalParameter<std::shared_ptr<Function>>(
    "function_handle", nullptr, "SpatialMaterialFunction object to be used to define the source.");
...
VolumetricSource::VolumetricSource(const InputParameters& params)
  : block_ids_(params.GetParamVectorValue<int>("block_ids")),
    logvol_(params.IsParameterValid("logical_volume")
              ? params.GetParamValue<std::shared_ptr<LogicalVolume>>("logical_volume")
              : nullptr),
    strength_(params.GetParamVectorValue<double>("group_strength")),
    function_(params.IsParameterValid("function_handle")
                ? std::dynamic_pointer_cast<VectorSpatialFunction>(
