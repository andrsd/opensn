  ...
  : ChiObject(params), id_(id), psi_uk_man_(app)
{
  ...
  quadrature_ = App().GetAngularQuadrature(quad_handle, fname);
...
lbs::LBSGroupset::BuildSubsets()
{
  grp_subset_infos_ = chi::MakeSubSets(groups_.size(), master_num_grp_subsets_);
  {
    ...
    {
      App().Log().Log() << "Groupset " << id_ << " has group-subset " << ss << " " << info.ss_begin
                        << "->" << info.ss_end;
      ++ss;
    }
  }
...
lbs::LBSGroupset::PrintSweepInfoFile(size_t ev_tag, const std::string& file_name)
{
  ...
  // Print event history
  ofile << App().Log().PrintEventHistory(ev_tag);
