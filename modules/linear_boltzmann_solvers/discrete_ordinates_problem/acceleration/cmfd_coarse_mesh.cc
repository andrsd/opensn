...
BuildGlobalFineCellInfoMap(const Mesh& grid)
{
  ...
  for (std::uint32_t cell_local_id = 0; cell_local_id < grid.GetLocalCellCount(); ++cell_local_id)
  {
    ...
    local_keys.push_back(static_cast<uint64_t>(grid.GetCellFaceCount(grid.MapCellGlobalID2LocalID(cell.global_id))));
...
CMFDCoarseMesh::BuildExteriorFaces(const Mesh& grid)
{
  ...
  for (auto& coarse_cell : local_cells_)
  {
    ...
    {
      const auto& fine_cell = grid.GetGlobalCell(fine_cell_id);
      for (std::size_t f = 0; f < grid.GetCellFaceCount(grid.MapCellGlobalID2LocalID(fine_cell.global_id)); ++f)
      {
...
CMFDCoarseMesh::BuildIdentity(const Mesh& grid)
{
       ...
       ++fine_cell_local_id)
  {
    ...
    coarse_cell.faces.reserve(grid.GetCellFaceCount(grid.MapCellGlobalID2LocalID(fine_cell.global_id)));

    auto faces = grid.GetCellFaces(grid.MapCellGlobalID2LocalID(fine_cell.global_id));
                                       ...
                                       const std::size_t target_fine_cells_per_coarse_cell)
{
  ...
  for (const auto& aggregate : aggregates)
  {
    ...
    for (const auto fine_cell_id : aggregate.fine_cell_ids)
    {
      ...
      {
        const auto& fine_face = grid.GetCellFace(cell_local_id, f_idx);
        ...
        coarse_face.fine_faces.push_back(
          {fine_cell.global_id,
           ...
           f_idx,
           fine_face.has_neighbor ? std::optional<uint64_t>(fine_face.neighbor_id) : std::nullopt,
           fine_face.has_neighbor ? grid.GetGlobalCell(fine_face.neighbor_id).partition_id : fine_cell.partition_id});
