                                  ...
                                  std::vector<std::pair<int, short>>& delayed_lock_box)
{
  ...
  for (auto f = 0; f < cell.faces.size(); ++f)
  {
    ...
    if (orientation != FaceOrientation::INCOMING or !face.IsNeighborLocal(grid_ptr.get()))
      continue;

    const auto num_face_dofs = face.vertex_ids.size();
    const auto face_categ_sz = grid_face_histogram.MapFaceHistogramBins(num_face_dofs);
    const auto face_categ = face_categ_sz;
    ...
    auto& lock_box = lock_boxes[face_categ_sz];
    ...
    if (!found)
    {
      ...
      oss << "AAH_FLUDSCommonData::SlotDynamics: Lock-box location not found.\n"
          ...
          << "Category: " << std::to_string(face_categ_sz) << ", "
  ...
  for (auto f = 0; f < cell.faces.size(); ++f)
  {
    ...
    const auto num_face_dofs = face.vertex_ids.size();
    const auto face_categ_sz = grid_face_histogram.MapFaceHistogramBins(num_face_dofs);
    outb_face_face_category.push_back(static_cast<short>(face_categ_sz));

    // Choose target lock box (default non-delayed)
    auto* target_lock_box = &lock_boxes[face_categ_sz];
    ...
    // Non-local outgoing
    if (face.has_neighbor and !face.IsNeighborLocal(grid_ptr.get()))
    {
