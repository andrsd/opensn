    ...
    for (auto& face : cell.faces)
    {
      ...
      if (cell_face_orientations_[cell_local_id][f] == FOOUTGOING)
      {
        ...
        if (face_info[cell_local_id][f].has_neighbor)
        {
          ...
          if (face_info[cell_local_id][f].neighbor_local)
          {
            ...
            cell_successors[cell_local_id].insert(
              std::make_pair(face_info[cell_local_id][f].neighbor_local_id, weight));
          }
          else
            location_successors.insert(face_info[cell_local_id][f].neighbor_partition_id);
        }
      }
      ...
      else if (cell_face_orientations_[cell_local_id][f] == FOINCOMING)
      {
        ...
        if (face_info[cell_local_id][f].has_neighbor and
          ...
          location_dependencies.insert(face_info[cell_local_id][f].neighbor_partition_id);
      }
      ++f;
    } // for face
  } // for cell
