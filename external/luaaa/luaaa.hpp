...
#if defined(_MSC_VER)
#define RTTI_CLASS_NAME(a) typeid(a).name() // vc always has this operator even if RTTI was
                                            // disabled.
#elif __GXX_RTTI
...
inline size_t
LUAAA_DUMP_OBJECT(char* buffer, size_t buflen, lua_State* L, int idx, int indent = 0)
{
  char prefix[32] = {0};
  if (indent > sizeof(prefix) - 1) { indent = sizeof(prefix) - 1; }
        ...
        while (lua_next(L, idx))
        {
          ...
          if (buflen > wrote)
          {
            ...
            else
            {
              ...
              else { ret = snprintf(buffer + wrote, buflen - wrote, "..."); }
            }
            if (ret > 0) { wrote += ret; }
          }
          lua_pop(L, 1);
...
inline void
LUAAA_DUMP(lua_State* L, const char* name = "")
{
  printf(">>>>>>>>>>>>>>>>>>>>>>>>>[%s]\n", name);
  char buffer[4096] = {0};
...
struct LuaStack
{
...
#else
    luaL_argcheck(state,
                  LuaClass<T>::klassName != nullptr,
                  1,
                  (std::string("cpp class `") + RTTI_CLASS_NAME(T) + "` not export").c_str());
#endif
  ...
  inline static void put(lua_State* L, T* t) { lua_pushlightuserdata(L, t); }
};
...
struct LuaStack<T&&> : public LuaStack<T>
{
  inline static void put(lua_State* L, T&& t) { LuaStack<T>::put(L, std::forward<T>(t)); }
};
...
struct LuaStack<float>
{
  ...
  {
    if (lua_isnumber(L, idx) || lua_isstring(L, idx)) { return float(lua_tonumber(L, idx)); }
    else { luaL_checktype(L, idx, LUA_TNUMBER); }
    return 0;
  }

  inline static void put(lua_State* L, const float& t) { lua_pushnumber(L, t); }
};
...
struct LuaStack<double>
{
  ...
  {
    if (lua_isnumber(L, idx) || lua_isstring(L, idx)) { return double(lua_tonumber(L, idx)); }
    else { luaL_checktype(L, idx, LUA_TNUMBER); }
    return 0;
  }

  inline static void put(lua_State* L, const double& t) { lua_pushnumber(L, t); }
};
...
struct LuaStack<int>
{
  ...
  {
    if (lua_isnumber(L, idx) || lua_isstring(L, idx)) { return int(lua_tointeger(L, idx)); }
    else { luaL_checktype(L, idx, LUA_TNUMBER); }
    return 0;
  }

  inline static void put(lua_State* L, const int& t) { lua_pushinteger(L, t); }
};
...
struct LuaStack<bool>
{
  ...
  inline static void put(lua_State* L, const bool& t) { lua_pushboolean(L, t); }
};
...
struct LuaStack<const char*>
{
  ...
  inline static void put(lua_State* L, const char* s) { lua_pushstring(L, s); }
};
...
struct LuaStack<char*>
{
  ...
  inline static void put(lua_State* L, const char* s) { LuaStack<const char*>::put(L, s); }
};
...
{
  inline static lua_State* get(lua_State* L, int) { return L; }
...
// InputParameters
template <>
struct LuaStack<opensn::InputParameters>
{
  inline static opensn::InputParameters get(lua_State* L, int idx)
  {
    ...
    //    input_params.AssignParameters(result);
    //
    //    return input_params;
    return result;
  }
...
template <typename T>
inline void
LuaStackReturn(lua_State* L, T t)
{
  lua_settop(L, 0);
  LuaStack<T>::put(L, t);
}
    ...
    {                                                                                              \
      ...
      {                                                                                            \
        ...
        {                                                                                          \
          ...
          else { lua_pushnil(cacheLuaState); }                                                     \
...
struct LuaStack<std::function<RET(ARGS...)>>
{
  ...
  inline static FTYPE get(lua_State* L, int idx)
  {
    ...
    struct HelperClass
    {
      static RET f_callback(ARGS... args)
      {
        ...
        {
          int initParams[] = {(LuaStack<ARGS>::put(cacheLuaState, args), 0)..., 0};
          if (lua_pcall(cacheLuaState, sizeof...(ARGS), 1, 0) != 0) { lua_error(cacheLuaState); }
          luaL_unref(cacheLuaState, LUA_REGISTRYINDEX, cacheLuaFuncId);
        }
        else { lua_pushnil(cacheLuaState); }
        return LuaStack<RET>::get(cacheLuaState, lua_gettop(cacheLuaState));
  ...
  inline void put(lua_State* L, FTYPE f) { lua_pushcfunction(L, NonMemberFunctionCaller(f)); }
};
...
TRET
LuaInvokeImpl(lua_State* state, void* calleePtr, size_t skip, indices<Ns...>)
{
...
inline TRET
LuaInvoke(lua_State* state, void* calleePtr, size_t skip)
{
  return LuaInvokeImpl<TRET, FTYPE, ARGS...>(
    state, calleePtr, skip, typename make_indices<sizeof...(ARGS)>::type());
}
  ...
  {                                                                                                \
    ...
    {                                                                                              \
      ...
      {                                                                                            \
        ...
        if (calleePtr)                                                                             \
        {                                                                                          \
          LuaStackReturn<TRET>(state,                                                              \
                               LuaInvoke<TRET, FTYPE, ARGS...>(state, calleePtr, SKIPPARAM));      \
          return 1;                                                                                \
        }                                                                                          \
        return 0;                                                                                  \
      }                                                                                            \
  ...
  {                                                                                                \
    ...
    {                                                                                              \
      ...
      {                                                                                            \
        void* calleePtr = lua_touserdata(state, lua_upvalueindex(1));                              \
        luaL_argcheck(state, calleePtr, 1, "cpp closure function not found.");                     \
        if (calleePtr) { LuaInvoke<void, FTYPE, ARGS...>(state, calleePtr, SKIPPARAM); }           \
        return 0;                                                                                  \
      }                                                                                            \
    };                                                                                             \
    return HelperClass::Invoke;                                                                    \
...
TRET
LuaInvokeInstanceMemberImpl(lua_State* state, void* calleePtr, indices<Ns...>)
{
  return (LuaStack<TCLASS>::get(state, 1).*
          *(FTYPE*)(calleePtr))(LuaStack<ARGS>::get(state, Ns + 2)...);
}

template <typename TCLASS, typename TRET, typename FTYPE, typename... ARGS>
inline TRET
LuaInvokeInstanceMember(lua_State* state, void* calleePtr)
{
  return LuaInvokeInstanceMemberImpl<TCLASS, TRET, FTYPE, ARGS...>(
    state, calleePtr, typename make_indices<sizeof...(ARGS)>::type());
}

template <typename TCLASS, typename TRET, typename... ARGS>
lua_CFunction
MemberFunctionCaller(TRET (TCLASS::*func)(ARGS...))
{
  typedef decltype(func) FTYPE;
  (void)(func);
  struct HelperClass
  {
    static int Invoke(lua_State* state)
    {
      ...
      {
        LuaStackReturn<TRET>(
          state, LuaInvokeInstanceMember<TCLASS, TRET, FTYPE, ARGS...>(state, calleePtr));
        return 1;
      }
      return 0;
    }
...
template <typename TCLASS, typename TRET, typename... ARGS>
lua_CFunction
MemberFunctionCaller(TRET (TCLASS::*func)(ARGS...) const)
{
  typedef decltype(func) FTYPE;
  (void)(func);
  struct HelperClass
...
lua_CFunction
MemberFunctionCaller(void (TCLASS::*func)(ARGS...))
{
  typedef decltype(func) FTYPE;
  (void)(func);
  struct HelperClass
  {
    static int Invoke(lua_State* state)
    {
      void* calleePtr = lua_touserdata(state, lua_upvalueindex(1));
      luaL_argcheck(state, calleePtr, 1, "cpp closure function not found.");
      if (calleePtr) { LuaInvokeInstanceMember<TCLASS, void, FTYPE, ARGS...>(state, calleePtr); }
      return 0;
    }
  };
  return HelperClass::Invoke;
...
template <typename TCLASS, typename... ARGS>
lua_CFunction
MemberFunctionCaller(void (TCLASS::*func)(ARGS...) const)
{
  typedef decltype(func) FTYPE;
  (void)(func);
  struct HelperClass
...
template <typename TRET, typename... ARGS>
lua_CFunction
MemberFunctionCaller(const std::function<TRET(ARGS...)>& func)
{
...
template <typename... ARGS>
lua_CFunction
MemberFunctionCaller(const std::function<void(ARGS...)>& func)
{
  typedef std::function<void(ARGS...)> FTYPE;
  (void)(func);
  struct HelperClass
  {
    static int Invoke(lua_State* state)
    {
      void* calleePtr = lua_touserdata(state, lua_upvalueindex(1));
      luaL_argcheck(state, calleePtr, 1, "cpp closure function not found.");
      if (calleePtr) { LuaInvoke<void, FTYPE, ARGS...>(state, calleePtr, 1); }
      return 0;
    }
  };
  return HelperClass::Invoke;
...
template <typename TRET, typename... ARGS>
lua_CFunction
NonMemberFunctionCaller(const std::function<TRET(ARGS...)>& func)
{
  typedef std::function<TRET(ARGS...)> FTYPE;
  (void)(func);
  struct HelperClass
...
template <typename... ARGS>
lua_CFunction
NonMemberFunctionCaller(const std::function<void(ARGS...)>& func)
{
  typedef std::function<void(ARGS...)> FTYPE;
  (void)(func);
  struct HelperClass
  {
    static int Invoke(lua_State* state)
    {
      void* calleePtr = lua_touserdata(state, lua_upvalueindex(1));
      luaL_argcheck(state, calleePtr, 1, "cpp closure function not found.");
      if (calleePtr) { LuaInvoke<void, FTYPE, ARGS...>(state, calleePtr, 0); }
      return 0;
    }
  };
  return HelperClass::Invoke;
...
struct FactoryConstructorCaller
{
  static TCLASS* Invoke(lua_State* state, void* mem, size_t skip)
  {
    return InvokeImpl(state, mem, typename make_indices<1>::type(), skip);
  }
...
private:
  ...
  static TCLASS* InvokeImpl(lua_State* state, void* mem, indices<Ns...>, size_t skip)
  {
    opensn::InputParameters input_params = TCLASS::GetInputParameters();
    opensn::InputParameters ip2 = LuaStack<opensn::InputParameters>::get(state, 1 + skip);

    auto type = LuaClass<TCLASS, TAG>::klassName;
    input_params.SetObjectType(type);
    input_params.SetErrorOriginScope(type);
    input_params.AssignParameters(ip2);

    return new (mem) TCLASS(input_params);
//    return new (mem) TCLASS(LuaStack<opensn::InputParameters>::get(state, Ns + 1 + skip)...);
  }
...
{
  static void Invoke(TCLASS* obj) { delete obj; }
};
...
struct LuaClass
{
  ...
  friend struct FactoryConstructorCaller<TCLASS, TAG>;
  template <typename>
...
public:
  LuaClass(lua_State* state, const char* name, const luaL_Reg* functions = nullptr) : m_state(state)
  {
    ...
    luaL_argcheck(state,
                  (klassName == nullptr),
                  1,
                  "LuaCalss<CLASS> name conflict, use LuaClass<CLASS, TAG> to identify them");
#else
    luaL_argcheck(state,
                  (klassName == nullptr),
                  1,
                  (std::string("C++ class `") + RTTI_CLASS_NAME(TCLASS) +
                   "` bind to conflict lua name `" + name + "`, origin name: `" + klassName +
                   "`. use use LuaClass<CLASS, TAG> to identify them.")
                    .c_str());
#endif
    ...
    struct HelperClass
    {
      ...
      static int f__objgc(lua_State* state)
      {
        ...
        {
          auto uData =
            (UserDataDetail*)luaL_checkudata(state, -1, (LuaClass<TCLASS, TAG>::klassName));
          if (uData && uData->dtor) { return (uData->dtor)(uData); }
        }
        return 0;
      }
    };
    ...
    if (functions) { luaL_setfuncs(state, functions, 0); }
  ...
  inline LuaClass<TCLASS, TAG>& ctor(const char* name = "new")
  {
    struct HelperClass
    {
      ...
      {
        if (uData && uData->obj) { (uData->obj)->~TCLASS(); }
        return 0;
      }

      static int f_new(lua_State* state)
      {
        auto uData = (LuaClass<TCLASS, TAG>::UserDataDetail*)lua_newuserdata(
          state, sizeof(LuaClass<TCLASS, TAG>::UserDataDetail) + sizeof(TCLASS));
        if (uData)
        {
          TCLASS* obj =
            PlacementConstructorCaller<TCLASS, ARGS...>::Invoke(state, (void*)(uData + 1), 0);
          if (obj)
          {
            uData->obj = obj;
  ...
  inline LuaClass<TCLASS, TAG>&
  ctor2(const char* name)
  {
    struct HelperClass
    {
      static int f_dtor(LuaClass<TCLASS, TAG>::UserDataDetail* uData)
      ...
      static int f_new(lua_State* state)
      {
        ...
        {
          TCLASS* obj =
            FactoryConstructorCaller<TCLASS, TAG>::Invoke(state, (void*)(uData + 1), 0);
//          TCLASS* obj =
//            PlacementConstructorCaller<TCLASS, ARGS...>::Invoke(state, (void*)(uData + 1), 0);
          if (obj)
          {
  ...
  inline LuaClass<TCLASS, TAG>& ctor(const char* name, TCLASS* (*spawner)(ARGS...))
  {
    ...
    struct HelperClass
    {
      static int f_dtor(LuaClass<TCLASS, TAG>::UserDataDetail* uData)
      {
        if (uData && uData->obj) { DestructorCaller<TCLASS>::Invoke(uData->obj); }
        return 0;
      }
      ...
      static int f_new(lua_State* state)
      {
        ...
        {
          auto uData = (LuaClass<TCLASS, TAG>::UserDataDetail*)lua_newuserdata(
            state, sizeof(LuaClass<TCLASS, TAG>::UserDataDetail));
          if (uData)
          {
...
#if LUAAA_WITHOUT_CPP_STDLIB
    luaL_argcheck(
      m_state, spawnerPtr != nullptr, 1, "faild to alloc mem to store spawner for ctor");
#else
    luaL_argcheck(
      m_state,
      spawnerPtr != nullptr,
      1,
      (std::string("faild to alloc mem to store spawner for ctor `") + name + "`").c_str());
#endif
    if (!spawnerPtr)
    {
      lua_pop(m_state, 1);
  ...
  template <typename TRET, typename... ARGS>
  inline LuaClass<TCLASS, TAG>&
  ctor(const char* name, TCLASS* (*spawner)(ARGS...), TRET (*deleter)(TCLASS*))
  {
    typedef decltype(spawner) SPAWNERFTYPE;
...
#if LUAAA_WITHOUT_CPP_STDLIB
    luaL_argcheck(
      m_state, spawnerPtr != nullptr, 0, ("faild to alloc mem to store spawner for ctor"));
#else
    luaL_argcheck(
      m_state,
      spawnerPtr != nullptr,
...
#if LUAAA_WITHOUT_CPP_STDLIB
    luaL_argcheck(
      m_state, deleterPtr != nullptr, 0, ("faild to alloc mem to store deleter for ctor"));
#else
    luaL_argcheck(
      m_state,
      deleterPtr != nullptr,
      0,
      (std::string("faild to alloc mem to store deleter for ctor `") + name + "`").c_str());
#endif
    if (!deleterPtr)
    {
      lua_pop(m_state, 3);
  ...
  inline LuaClass<TCLASS, TAG>& ctor(const char* name, TCLASS* (*spawner)(ARGS...), std::nullptr_t)
  {
    ...
    SPAWNERFTYPE* spawnerPtr = (SPAWNERFTYPE*)lua_newuserdata(m_state, sizeof(SPAWNERFTYPE));
#if LUAAA_WITHOUT_CPP_STDLIB
    luaL_argcheck(m_state,
                  spawnerPtr != nullptr,
                  1,
                  "faild to alloc mem to store spawner for ctor of cpp class");
#else
    luaL_argcheck(
      m_state,
      spawnerPtr != nullptr,
  ...
  template <typename TRET, typename... ARGS>
  inline LuaClass<TCLASS, TAG>&
  ctor(const std::string& name, TCLASS* (*spawner)(ARGS...), TRET (*deleter)(TCLASS*))
  {
  ...
  template <typename... ARGS>
  inline LuaClass<TCLASS, TAG>&
  ctor(const std::string& name, TCLASS* (*spawner)(ARGS...), std::nullptr_t)
  {
...
private:
  ...
  inline LuaClass<TCLASS, TAG>& _funImpl(const char* name, F f)
  {
    ...
    luaL_argcheck(m_state,
                  funPtr != nullptr,
                  1,
                  (std::string("faild to alloc mem to store function `") + name + "`").c_str());
#endif
    if (funPtr)
    {
...
{
public:
  LuaModule(lua_State* state, const char* name = "_G") : m_state(state)
  {
    size_t strBufLen = strlen(name) + 1;
...
public:
  ...
  template <typename TCLASS, int TAG>
  inline LuaModule& def(const char* name,
                        const luaaa::LuaClass<TCLASS, TAG>&,
                        const TCLASS* obj = nullptr,
                        void (*deleter)(TCLASS*) = nullptr)
  {
    ...
    auto uData = (typename LuaClass<TCLASS, TAG>::UserDataDetail*)lua_newuserdata(
      m_state, sizeof(LuaClass<TCLASS, TAG>::UserDataDetail));
#if LUAAA_WITHOUT_CPP_STDLIB
    ...
    luaL_argcheck(m_state,
                  uData != nullptr,
                  1,
                  (std::string("faild to alloc mem to store object `") + name + "`").c_str());
#endif
    if (uData)
    {
...
struct LuaStack<std::map<K, V, ARGS...>>
{
  ...
  inline static Container get(lua_State* L, int idx)
  {
    ...
    if (lua_istable(L, idx))
    {
      ...
      {
        const int top = lua_gettop(L);
        result[LuaStack<typename Container::key_type>::get(L, top - 1)] =
          LuaStack<typename Container::mapped_type>::get(L, top);
        lua_pop(L, 1);
      }
      lua_pop(L, 0);
    }
