...
chi_math_SDM_Test02_DisContinuous(const chi::InputParameters& input_parameters)
{
  ...
  //             aij +=
  //               kappa * fqp_data.ShapeValue(i, qp) * fqp_data.ShapeValue(jm, qp) *
  //               fqp_data.JxW(qp);
  //
  ...
  //           for (size_t qp : fqp_data.QuadraturePointIndices())
  //             vec_aij += fqp_data.ShapeValue(jm, qp) * fqp_data.ShapeGrad(i, qp) *
  //             fqp_data.JxW(qp);
  ...
  //           chi_mesh::Vector3 vec_aij;
  //           for (size_t qp : fqp_data.QuadraturePointIndices())
  //             vec_aij += fqp_data.ShapeValue(im, qp) * fqp_data.ShapeGrad(j, qp) *
  //             fqp_data.JxW(qp);
  //           const double aij = -0.5 * D * n_f.Dot(vec_aij);
  //
