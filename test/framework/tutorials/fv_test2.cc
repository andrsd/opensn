...
chiSimTest02_FV(const chi::InputParameters&)
{
  ...
  //
  // chi_math::VectorGhostCommunicator vgc(num_local_dofs, num_globl_dofs, ghost_ids,
  // Chi::mpi.comm); std::vector<double> field_wg = vgc.MakeGhostedVector(field);
  //
  ...
  //     }
  //
  //     grad_phi_P += Af * ((xn - xf).Norm() * phi_P + (xf - xp).Norm() * phi_N) / (xn -
  //     xp).Norm();
  //     ++f;
  //   } // for face
