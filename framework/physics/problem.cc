...
Problem::Problem(std::string name, std::initializer_list<BasicOption> options)
  : basic_options_(options), name_(std::move(name))
{
}
...
BasicOptions&
Problem::GetBasicOptions()
{
  return basic_options_;
}

const BasicOptions&
Problem::GetBasicOptions() const
{
  return basic_options_;
}
