...
std::tuple<Cell, std::vector<uint64_t>, std::vector<std::vector<uint64_t>>, std::vector<CellFace>>
           ...
           const size_t k)
{
  ...
  faces_for_this_cell.push_back(left_face);
  faces_for_this_cell.push_back(right_face);
  cell_face_vertex_ids.push_back({cell_vertex_ids[0]});
  cell_face_vertex_ids.push_back({cell_vertex_ids[1]});

  return {cell, cell_vertex_ids, cell_face_vertex_ids, faces_for_this_cell};
}
           ...
           const size_t k)
{
  ...
  std::vector<std::vector<uint64_t>> cell_face_vertex_ids;
  std::vector<CellFace> faces_for_this_cell;
  auto add_face = [&faces_for_this_cell, &cell_face_vertex_ids](std::vector<uint64_t> vertex_ids,
...
OrthogonalMeshGenerator::Execute()
{
  ...
  {
    auto [cell, cell_vertex_ids, cell_face_vertex_ids, faces_for_this_cell] =
  ...
  for (const auto& cell : local_cells)
    for (auto& face : cell_faces[cell.global_id])
      mesh_faces.push_back(std::move(face));
                                                        ...
                                                        const CoordinateSystemType coord_sys)
{
  ...
  for (size_t i = 0; i < Ny - 1; ++i)
  {
    for (size_t j = 0; j < Nx - 1; ++j)
    {
      ...
      for (auto& f : faces_for_this_cell)
        mesh_faces.push_back(std::move(f));
      cells.emplace_back(cell);
      cell_connect.emplace_back(cell_vertex_ids);
      cell_face_connect.emplace_back(std::move(cell_face_vertex_ids));
    }
