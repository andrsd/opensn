...
SplitFileMeshGenerator::SplitFileMeshGenerator(opensn::App& app, const chi::InputParameters& params)
...
{
  const int num_mpi = App().ProcessCount();
  const int num_parts = num_mpi == 1 ? num_parts_ : num_mpi;

  if (App().LocationID() == 0 and (not read_only_))
  {
    ...
    App().Log().Log() << "Writing split-mesh with " << num_parts << " parts";
    const auto cell_pids = PartitionMesh(*current_umesh, num_parts);
    WriteSplitMesh(cell_pids, *current_umesh, num_parts);
    App().Log().Log() << "Split-mesh with " << num_parts << " parts successfully created";
  } // if home location

  // Other locations wait here for files to be written
  App().Barrier();

  if (App().ProcessCount() == num_parts)
  {
    App().Log().Log() << "Reading split-mesh";
    auto mesh_info = ReadSplitMesh();

    auto grid_ptr = SetupLocalMesh(App(), mesh_info);
    ...
    App().Log().Log() << "Done reading split-mesh files";
  }
  else
  {
    App().Log().Log0Warning()
      ...
      << App().Log().GetTimingBlock("ChiTech").MakeGraphString();
    opensn::App::Exit(EXIT_SUCCESS);
  }
                                       ...
                                       int num_parts)
{
  ...
  auto& t_write = App().Log().CreateOrGetTimingBlock("FileMeshGenerator::WriteSplitMesh");
  auto& t_sorting =
    App().Log().CreateOrGetTimingBlock("Sorting data", "FileMeshGenerator::WriteSplitMesh");
  auto& t_cells =
    App().Log().CreateOrGetTimingBlock("WriteCells", "FileMeshGenerator::WriteSplitMesh");
  auto& t_verts =
    App().Log().CreateOrGetTimingBlock("WriteVerts", "FileMeshGenerator::WriteSplitMesh");
  auto& t_serialize = App().Log().CreateOrGetTimingBlock("Serialize");
  ...
  for (int pid = 0; pid < num_parts; ++pid)
  {
    ...
    if (fraction_complete >= static_cast<double>(aux_counter + 1) * 0.1)
    {
      ...
      //   App().Log().Log() << Chi::program_timer.GetTimeString() << " Surpassing part " << pid
      //                     << " of " << num_parts << " (" << (aux_counter + 1) * 10 << "%)";
      ++aux_counter;
    }
  } // for p
}
...
SplitFileMeshGenerator::ReadSplitMesh()
{
  ...
  ChiLogicalErrorIf(App().ProcessCount() != file_num_parts,
                    "Split mesh files with prefix \"" + split_file_prefix_ +
                      "\" has been created with " + std::to_string(file_num_parts) +
                      " parts but is now being read with " + std::to_string(App().ProcessCount()) +
                      " processes.");
...
SplitFileMeshGenerator::SetupLocalMesh(opensn::App& app, SplitMeshInfo& mesh_info)
{
