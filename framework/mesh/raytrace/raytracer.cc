                     ...
                     RayTracerOutputInformation& oi)
{
  ...
  const double fabs_mu = std::fabs(omega_i.Dot(grid->GetCellFace(cell_local_id, 0).normal));
  ...
  for (int f = 0; f < num_faces; ++f)
  {
    ...
    bool intersects = CheckPlaneLineIntersect(
      grid->GetCellFace(cell_local_id, f).normal, face_point, pos_i, pos_f_line, intersection_point, &weights);
    ...
    if ((D > backward_tolerance_) and intersects)
    {
      ...
      oi.destination_face_index = f;
      oi.destination_face_neighbor = grid->GetCellFace(cell_local_id, f).neighbor_id;
                        ...
                        RayTracerOutputInformation& oi)
{
  ...
  {
    if (grid->GetCellFace(cell_local_id, f).normal.Dot(omega_i) < 0.0)
      continue;
    ...
    bool intersects = CheckLineIntersectStrip(
      face_point_i, face_point_f, grid->GetCellFace(cell_local_id, f).normal, pos_i, pos_f_line, ip);
    ...
    if ((D > backward_tolerance_) and intersects)
    {
      ...
      face_oi.destination_face_neighbor = grid->GetCellFace(cell_local_id, f).neighbor_id;
      intersection_found = true;
                          ...
                          std::vector<double>& segment_lengths)
{
  ...
  {
    for (size_t f = 0; f < grid->GetCellFaceCount(cell_local_id); ++f) // edges
    {
      const auto& face = grid->GetCellFace(cell_local_id, f);
      auto face_vertex_ids = grid->GetCellFaceConnectivity(cell_local_id, f);
      const auto& v0 = grid->GlobalVertex(face_vertex_ids[0]);
