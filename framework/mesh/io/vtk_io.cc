...
std::tuple<Cell, std::vector<std::uint64_t>, std::vector<std::vector<std::uint64_t>>, std::vector<CellFace>>
CreateCellFromVTKPolyhedron(vtkCell* vtk_cell)
{
  ...
  switch (sub_type)
  {
    ...
    default:
    {
      faces_for_this_cell.reserve(num_cfaces);
  ...
  return {polyh_cell, polyh_cell_vertex_ids, cell_face_vertex_ids, faces_for_this_cell};
}
...
CreateCellFromVTKPolygon(vtkCell* vtk_cell)
{
  ...
  return {poly_cell, polyh_cell_vertex_ids, cell_face_vertex_ids, faces_for_this_cell};
}
...
CreateCellFromVTKLine(vtkCell* vtk_cell)
{
  ...
  return {slab_cell, slab_cell_vertex_ids, cell_face_vertex_ids, faces_for_this_cell};
}
...
CreateCellFromVTKVertex(vtkCell* vtk_cell)
{
  ...
  return {point_cell, point_cell_vertex_ids, empty_cell_face_vertex_ids, empty_faces};
}
                        ...
                        const std::string& block_id_array_name)
{
  ...
  if (has_global_ids)
  {
    ...
    for (vtkIdType c = 0; c < total_cell_count; ++c)
    {
      ...
      std::vector<CellFace> faces_for_this_cell;
      if (vtk_celldim == 3)
        std::tie(raw_cell, raw_cell_vertex_ids, raw_cell_face_vertex_ids, faces_for_this_cell) = CreateCellFromVTKPolyhedron(vtk_cell);
      else if (vtk_celldim == 2)
        std::tie(raw_cell, raw_cell_vertex_ids, raw_cell_face_vertex_ids, faces_for_this_cell) = CreateCellFromVTKPolygon(vtk_cell);
      else if (vtk_celldim == 1)
        std::tie(raw_cell, raw_cell_vertex_ids, raw_cell_face_vertex_ids, faces_for_this_cell) = CreateCellFromVTKLine(vtk_cell);
      else if (vtk_celldim == 0)
        std::tie(raw_cell, raw_cell_vertex_ids, raw_cell_face_vertex_ids, faces_for_this_cell) = CreateCellFromVTKVertex(vtk_cell);
      else
  ...
  else
  {
    ...
    for (vtkIdType c = 0; c < total_cell_count; ++c)
    {
      ...
      std::vector<CellFace> faces_for_this_cell;
      if (vtk_celldim == 3)
        std::tie(cell, cell_vertex_ids, cell_face_vertex_ids, faces_for_this_cell) = CreateCellFromVTKPolyhedron(vtk_cell);
      else if (vtk_celldim == 2)
        std::tie(cell, cell_vertex_ids, cell_face_vertex_ids, faces_for_this_cell) = CreateCellFromVTKPolygon(vtk_cell);
      else if (vtk_celldim == 1)
        std::tie(cell, cell_vertex_ids, cell_face_vertex_ids, faces_for_this_cell) = CreateCellFromVTKLine(vtk_cell);
      else if (vtk_celldim == 0)
        std::tie(cell, cell_vertex_ids, cell_face_vertex_ids, faces_for_this_cell) = CreateCellFromVTKVertex(vtk_cell);
      else
        throw std::logic_error(fname + ": Unsupported cell dimension.");
