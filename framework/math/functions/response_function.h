...
public:
   ...
   * \param pos The xyz coordinates of the point where the function is evaluated.
   * \param mat_id Material ID
   * \return Vector with the response (should have `num_groups` entries)
   */
  virtual std::vector<double> Evaluate(int num_groups, const Vector3& pos, int mat_id) const = 0;
};
